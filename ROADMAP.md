...
- **The x86_64 port** — the HAL's honesty test. ✅ Stage 1 (landed
  ...
  lacks them) and a TLB scheme nobody has run does not land; an I/O
  APIC and MSI-X for guests. ✅ The framebuffer console (2026-09-04):
  the loader's framebuffer, mapped write-combining, draws every log
  line beside the serial port (`kernel/fbcon.zig`, an 8x16 font from
  Departure Mono). Modern hardware only: no legacy PIC, PIT, or BIOS
...
- **mshl v3 — the language as a tool for the fabric** (decisions in the
  ...
  ✅ its residuals (landed 2026-09-04, "stage 6"): shape annotations
  checked where they run, host-command signatures derived from the
...
- **The graphical console: a display server, surfaces, and input**
  ...
  - **The mshl GUI layer** (the arc's whole point — GUIs in mshl, not
    ...
    is a declarative widget tree carrying event ids (not closures), so
    only data crosses and it is supervised/crash-only/fabric-shippable.
    ✅ Stage 1 (landed 2026-09-08): `gui { init, view, update }` is a
    hosted mshl command (user/guicmds.zig) in mshrun, gated on a display
    cap; it owns a surface, renders a column of label/button widgets
    (client-side, shared font), routes the keyboard (Tab between buttons,
...
- ✅ **Network settings, stage 3: persisted, and in Settings**
  ...
  their rows (the two-row interface table hid its second row); a
  `tabs` strip with `bar: true` at the root is window chrome, flush
  under the titlebar with the page padded below it.
...
- ✅ **The framebuffer console** (2026-09-04): `kernel/fbcon.zig` draws
  ...
  entry 1, an 8x16 bitmap rasterized from Departure Mono (OFL 1.1,
  `kernel/font/`, regenerated by `tools/mkfont.py`), a text shadow that
  a scroll redraws from, UTF-8 sequences drawn as one stand-in glyph,
  a bounded lock so a panic never hangs on it. Verified by QEMU
  screendump (1280x800, 160x50 characters, the boot log and a panic
  legible; a scrolled screenful correct); the panic drill asserts the
  console came up on x86_64. aarch64's devicetree boot brings no
  framebuffer; its `Info` carries the field for one. Found: `pcisvc`
  re-assigned every BAR from the window base, UEFI's placements
  included, so the framebuffer moved out from under the kernel and
  each store became a VM exit (a five-second scroll; three drills
  stalled a minute at a time) — firmware's BAR placements are now kept
  where they lie inside the window.
