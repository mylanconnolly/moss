...
fn hostSignature(_: *anyopaque, name: []const u8) ?mshl.Signature {
    ...
    if (confcmds.on()) {
        if (confcmds.signature(name)) |sig| return sig;
    }
    if (localecmds.on()) {
...
fn hostCall(_: *anyopaque, it: *mshl.Interp, name: []const u8, args: []const Value, input: ?Value) mshl.Error!?Value {
    ...
    if (confcmds.on()) {
        if (try confcmds.call(it, name, args, input)) |v| return v;
    }
    if (localecmds.on()) {
...
export fn umain(log_h: u64, chan_h: u64, arg: u64, blob_va: u64, blob_len: u64) callconv(.c) noreturn {
    ...
    // needs, so `spawn` without a spawner, `dial` without a fabric, etc.
    // fail with a clear message rather than reading as an unknown command.
    // This also lets a fabric-free GUI (the local file explorer) name the
    // network commands: `net-rows` returns an empty peer list without a
    // fabric, so the same explorer script runs local-only or networked.
