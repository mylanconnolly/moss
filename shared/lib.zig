...
pub const Syscall = enum(u64) {
    ...
    cap_kind = 45,
    /// log_read(introspect, from, buf, len) -> x1 = bytes copied, x2 = the
    /// offset they start at (`from`, or later when the ring has dropped
    /// the older bytes — the reader then resumes at a line boundary),
    /// x3 = the log's head (the offset the next line will get). The
    /// kernel keeps the recent log — every line any domain or the kernel
    /// printed — in a ring; the introspect cap (or a spawner) reads it,
    /// the same authority that sees the ledger.
    log_read = 46,
    _,
};
