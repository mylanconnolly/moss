...
| `flogin` (also: a script on node 2 runs three pipeline stages on node 1 through the bulk transport; prints the remote home's write, warm-read and cold-read times) | Fabric login and the one home: node 1 applies the users and publishes its session manager, node 2 (a fresh disk, a console) joins and alice logs in there — her record fetched, her home leased on node 1 and mounted on node 2 through the bulk transport, a file written; then node 2's disk is wiped, both boot again, and the file is still there | flogin (two QEMUs on one segment, two boots) |
...
- `--arch x86_64` (what `zig build -Darch=x86_64 check` passes) boots
  ...
  All twenty-nine rows run there, with the port's own fault-dump and
  DMA-refusal markers, under KVM when the host has it; `--tcg` (build:
  `-Dtcg`) leaves KVM out, which is how the gate runs on a host of
