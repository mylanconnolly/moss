...
```sh
zig build check      # the whole test suite: 16 OS tests under QEMU + host unit tests (~75s)
...
| shell | msh scripted console session: typed pipelines (where/select/get over real listings), let/if/for/while/def, `>` redirection, data files, scripts and a startup script, tab completion, and `run` of content-addressed programs whose results are values | `zig build run-shell` (interactive) |
