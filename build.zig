...
pub fn build(b: *std.Build) void {
    ...
    const term_test = b.option(
        bool,
        "term-test",
        "Run the terminal drill: a glyph grid rendered on a surface, screendumped over QMP",
    ) orelse false;
    const input_test = b.option(
        bool,
        "input-test",
        "Run the virtio-input drill: key presses injected over QMP, decoded by the driver",
    ) orelse false;
    const seat_test = b.option(
        bool,
        "seat-test",
    ...
    const comp_test = b.option(
        bool,
        "comp-test",
        "Run the compositor drill: overlapping windowed surfaces, screendumped over QMP",
    ) orelse false;
    const focus_test = b.option(
        bool,
        "focus-test",
        "Run the focus drill: the compositor routes keys to the focused window",
    ) orelse false;
    const trust_test = b.option(
        bool,
        "trust-test",
        "Run the trusted-path drill: a login greeter claims the trusted path; a hostile client is refused and cannot capture the keyboard",
    ) orelse false;
    const readers_test = b.option(
        bool,
        "readers-test",
        "Run the concurrent-input drill: a client parks a next_input while another keeps committing — the compositor serves both",
    ) orelse false;
    const ptr_test = b.option(
        bool,
        "ptr-test",
        "Run the pointer driver drill: a virtio-input tablet's moves + click injected over QMP, decoded by inputsvc",
    ) orelse false;
    const pointer_test = b.option(
        bool,
        "pointer-test",
    ...
    const guishellro_test = b.option(
        bool,
        "guishellro-test",
    ...
    const localeupd_test = b.option(
        bool,
        "localeupd-test",
        "Run the locale auto-updater drill: fetch a fresher cldr.db over TLS and install it",
    ) orelse false;
    const desktop_test = b.option(
        bool,
        "desktop-test",
    ...
    const dock_test = b.option(
        bool,
        "dock-test",
        "Run the dock drill: a resident dock launches an app unit through init, which opens its window",
    ) orelse false;
    const listdemo_test = b.option(
        bool,
        "listdemo-test",
        "Run the list-widget drill: a scrollable, selectable columned list -- selection, scrolling, activation",
    ) orelse false;
    const explorer_test = b.option(
        bool,
        "explorer-test",
    ...
    const activity_test = b.option(
        bool,
        "activity-test",
        "Run the Activity drill: the task-manager app lists the session's units and force-quits one",
    ) orelse false;
    const console_test = b.option(
        bool,
        "console-test",
        "Run the Console drill: the log viewer reads the kernel's ring, filters, pauses and follows it",
    ) orelse false;
    const nodes_test = b.option(
        bool,
        "nodes-test",
        "Run the Nodes drill: two nodes, the app lists the fabric's membership and checks the peer",
    ) orelse false;
    const nodevm_test = b.option(
        bool,
        "nodevm-test",
        "Run the guest-node drill: the Nodes app starts a moss guest in a VM, which joins the fabric, answers, and is stopped",
    ) orelse false;
    ...
    const gui_test = b.option(
        bool,
        "gui-test",
        "Run the mshl GUI drill: a GUI defined in mshl as pure view/update, rendered and keyboard-driven",
    ) orelse false;
    const guilogin_test = b.option(
        bool,
        "guilogin-test",
    ...
    const lconsole_test = b.option(
        bool,
        "lconsole-test",
        "Run the login-with-console isolation drill: login opens an interactive msh on a graphical terminal",
    ) orelse false;
    const gisession_test = b.option(
        bool,
        "gisession-test",
    ...
    build_opts.addOption(bool, "comp_test", comp_test);
    build_opts.addOption(bool, "focus_test", focus_test);
    build_opts.addOption(bool, "trust_test", trust_test);
    ...
    build_opts.addOption(bool, "console_test", console_test);
    build_opts.addOption(bool, "nodes_test", nodes_test);
    build_opts.addOption(bool, "nodevm_test", nodevm_test);
    ...
    const user_progs = [_]struct { name: []const u8, src: []const u8 }{
        ...
        .{ .name = "term", .src = "user/term.zig" },
        .{ .name = "inputsvc", .src = "user/inputsvc.zig" },
        ...
        .{ .name = "fontpush", .src = "user/fontpush.zig" },
    ...
    const ts_have_static = if (std.Io.Dir.cwd().access(b.graph.io, ts_static, .{})) true else |_| false;
    {
        ...
        // The lint reads the application-metadata vocabulary from the
        // same shared module init parses it with, so the two cannot drift.
        const shared_host_mod = b.createModule(.{ .root_source_file = b.path("shared/lib.zig"), .target = host_target, .optimize = .Debug });
        ...
        const base_imports = [_]std.Build.Module.Import{ .{ .name = "mshtree", .module = tree_mod }, .{ .name = "mshl", .module = mshl_mod }, .{ .name = "shared", .module = shared_host_mod } };
    ...
    for ([_][]const u8{
        ...
        "conf/units/dnsd-cluster.msh",    "conf/dns.msh",
        "conf/units/clock.msh",           "conf/clock.msh",
        ...
        "conf/units/gpucli.msh",          "conf/units/term.msh",
        "conf/units/inputsvc.msh",        "conf/units/kbd.msh",
        "conf/units/termsvc.msh",         "conf/units/gsh.msh",
        "conf/units/gshell.msh",          "conf/units/compcli.msh",
        "conf/units/compositor.msh",      "conf/units/focuscli.msh",
        "conf/units/trust-greeter.msh",   "conf/units/trust-fake.msh",
        "conf/trust.token",               "conf/units/readers-reader.msh",
        "conf/units/readers-mover.msh",   "conf/units/gui-demo.msh",
        "scripts/gui-demo.msh",           "conf/units/gui-listdemo.msh",
        ...
        "scripts/console.msh",            "conf/units/gui-console.msh",
        "conf/sessiongui/console.msh",    "scripts/nodes.msh",
        "conf/units/gui-nodes.msh",       "conf/sessiongui/nodes.msh",
        "conf/units/vmnode.msh",          "conf/units/gui-nodevm.msh",
        "conf/units/browse-client.msh",   "scripts/browse-cli.msh",
        "conf/units/gui-netbrowse.msh",   "conf/units/cascade-a.msh",
        "scripts/win-mid.msh",            "conf/units/cascade-b.msh",
        ...
        "conf/units/gui-click.msh",       "conf/units/fontpush.msh",
        "conf/userscale.msh",             "conf/units/usersvc-guishell.msh",
        "conf/units/gui-guishell.msh",    "conf/units/fabgui-app.msh",
        "scripts/fabgui.msh",             "conf/units/fabsig.msh",
        "conf/units/fabsigtx.msh",        "scripts/fabsig-wait.msh",
        "scripts/fabsig-send.msh",        "conf/units/fabname.msh",
        "scripts/fabname.msh",            "conf/units/locale-drill.msh",
        "scripts/locale-drill.msh",       "conf/units/localeupd.msh",
        "conf/locale.msh",                "conf/units/localesvc.msh",
        "conf/units/clipsvc.msh",         "conf/units/win-alpha.msh",
        "conf/units/win-beta.msh",        "scripts/win-alpha.msh",
        "scripts/win-beta.msh",           "conf/units/topbar.msh",
        "scripts/topbar.msh",             "conf/units/dock.msh",
        "scripts/dock.msh",               "conf/sessiongui/topbar.msh",
        "scripts/dtopbar.msh",            "conf/sessiongui/dock.msh",
        "scripts/ddock.msh",              "conf/sessiongui/settings.msh",
        "conf/sessiongui/win-demo.msh",   "conf/sessiongui/explorer.msh",
        "conf/sessiongui/sterm.msh",      "conf/sessiongui/terminal.msh",
        "conf/sessiongui/clipsvc.msh",    "scripts/demo.msh",
        "scripts/gui-shell.msh",          "conf/skel/font.msh",
    ...
    // aarch64, the ELF (debug info stripped) on x86_64.
    {
        ...
        for ([_][]const u8{
            ...
            "fontscale_test",  "guishell_test",  "guishellro_test", "display_test",
            "largetext_test",  "power_test",     "restart_test",    "fabgui_test",
    ...
    if (arch == .aarch64) {
        ...
        // Three NICs and two entropy devices. The first NIC is the cluster
        // segment (a hub, so the guishell profile's fabric address
        // 10.77.0.1 is real); the second is on the same hub and is a guest
        // node's — the Nodes app starts a moss kernel in a VM with it and
        // the second rng passed through, and the guest joins the fabric
        // as node 2; the third is QEMU's user network, leased by DHCP
        // (10.0.2.15, outbound through the host). Settings persists what
        // it applies, on the disk.
        const script = b.fmt(
            \\set -e
    ...
    const guieval_test_mod = b.createModule(.{
        .root_source_file = b.path("user/guieval.zig"),
        .target = host_target,
        .optimize = optimize,
    });
    guieval_test_mod.addImport("mosslib", lib_test_mod);
    const guieval_tests = b.addTest(.{ .root_module = guieval_test_mod });
    const test_step = b.step("test", "Run host-side unit tests");
    test_step.dependOn(&b.addRunArtifact(guieval_tests).step);
    ...
    const variants = [_][]const u8{
        ...
        "pointer",   "guiclick",  "fontscale", "guishell", "guishellro", "display",
