...
- `zig build -Darch=x86_64 run` — the x86_64 port (stage 1: boot to
  "boot complete", no interrupts/user mode yet) on OVMF + Limine, KVM
  here. The gate stays aarch64-only until the port has drills.
