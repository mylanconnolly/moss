...
//!   fn leafMeasure(t: *Tree, n: Node, avail_w: usize) Size;
//!   fn leafPaint(t: *Tree, n: Node, x: usize, y: usize, avail_w: usize) Size;
//!   fn childPaint(t: *Tree, n: Node, x: usize, y: usize, avail_w: usize) Size;
...
pub fn Engine(comptime Tree: type) type {
    return struct {
        ...
        pub fn measure(t: *Tree, n: Node, avail_w: usize) Size {
            return layout(t, n, 0, 0, avail_w, false);
        }
        pub fn paint(t: *Tree, n: Node, x: usize, y: usize, avail_w: usize) Size {
            return layout(t, n, x, y, avail_w, true);
        }
        ...
        fn row(t: *Tree, n: Node, x: usize, y: usize, avail_w: usize, do_paint: bool) Size {
            ...
            for (children) |child| {
                ...
                total += weight;
                if (weight == 0) fixed += layout(t, child, 0, 0, avail_w, false).w;
            }
            if (total > 0 and fixed < avail_w) {
                ...
                for (children) |child| {
                    ...
                    height = @max(height, layout(t, child, 0, 0, width, false).h);
                    before += weight;
                }
                before = 0;
                var xx = x;
            ...
            var row_h: usize = 0;
            for (children) |child| row_h = @max(row_h, layout(t, child, 0, 0, avail_w, false).h);
            for (children) |child| {
        ...
        fn column(t: *Tree, n: Node, x: usize, y: usize, avail_w: usize, do_paint: bool, section: bool) Size {
            const children = t.children(n);
            const g = t.gap(n);
            ...
            for (children, 0..) |child, i| {
                if (i > 0) height += g;
                height += layout(t, child, 0, 0, width, false).h;
            }
            if (do_paint) {
                ...
                for (children, 0..) |child, i| {
                    if (i > 0) yy += g;
                    yy += t.childPaint(child, x + inset, yy, width).h;
                }
            }
        ...
        /// stacked.
        fn split(t: *Tree, n: Node, x: usize, y: usize, avail_w: usize, do_paint: bool) Size {
            ...
            if (avail_w < left_w + split_gap + split_min_right) {
                const l = if (left) |v| (if (do_paint) t.childPaint(v, x, y, avail_w) else layout(t, v, 0, 0, avail_w, false)) else Size{};
                const r = if (right) |v| (if (do_paint) t.childPaint(v, x, y + l.h + split_gap, avail_w) else layout(t, v, 0, 0, avail_w, false)) else Size{};
                return .{ .w = avail_w, .h = l.h + split_gap + r.h };
            }
            const div = 1 + split_gap;
            const rw = avail_w -| (left_w + div);
            const l = if (left) |v| (if (do_paint) t.childPaint(v, x, y, left_w) else layout(t, v, 0, 0, left_w, false)) else Size{};
            const r = if (right) |v| (if (do_paint) t.childPaint(v, x + left_w + div, y, rw) else layout(t, v, 0, 0, rw, false)) else Size{};
            const h = @max(l.h, r.h);
            if (do_paint) t.dividerPaint(x + left_w + split_gap / 2, y, h);
            return .{ .w = avail_w, .h = h };
        }

        fn layout(t: *Tree, n: Node, x: usize, y: usize, avail_w: usize, do_paint: bool) Size {
            return switch (t.kind(n)) {
                .none => .{},
                .row => row(t, n, x, y, avail_w, do_paint),
                .column => column(t, n, x, y, avail_w, do_paint, false),
                .section => column(t, n, x, y, avail_w, do_paint, true),
                .scroll => blk: {
                ...
                .split => split(t, n, x, y, avail_w, do_paint),
                .leaf => if (do_paint) t.leafPaint(n, x, y, avail_w) else t.leafMeasure(n, avail_w),
            };
        }
    };
}
...
const TestTree = struct {
    ...
    fn leafMeasure(_: *TestTree, n: Node, avail_w: usize) Size {
        return .{ .w = @min(n.w, avail_w), .h = n.h };
    }
    fn leafPaint(t: *TestTree, n: Node, x: usize, y: usize, avail_w: usize) Size {
        t.paints[t.n] = .{ .node = n, .x = x, .y = y, .w = avail_w };
        t.n += 1;
        return .{ .w = @min(n.w, avail_w), .h = n.h };
    }
    fn childPaint(t: *TestTree, n: Node, x: usize, y: usize, avail_w: usize) Size {
        return Eng.paint(t, n, x, y, avail_w);
    }
    fn viewportPaint(t: *TestTree, _: Node, child: Node, x: usize, y: usize, w: usize, h: usize) Size {
        t.viewports += 1;
        _ = Eng.paint(t, child, x, y, w);
        return .{ .w = w, .h = h };
    }
...
test "a row wraps into lines of uniform height and centres children on them" {
    ...
    try std.testing.expectEqual(Size{ .w = 120, .h = 70 }, E.measure(&t, &r, 120));
    try std.testing.expectEqual(Size{ .w = 120, .h = 70 }, E.paint(&t, &r, 0, 100, 120));
...
test "flex children share the width left by fixed ones without drift" {
    ...
    var t: TestTree = .{};
    try std.testing.expectEqual(Size{ .w = 200, .h = 12 }, E.measure(&t, &r, 200));
    _ = E.paint(&t, &r, 0, 0, 200);
    ...
    try std.testing.expect(E.measure(&t, &r, 30).h > 12);
}
...
test "columns stack with gaps and sections inset inside a panel" {
    ...
    var t: TestTree = .{};
    try std.testing.expectEqual(Size{ .w = 100, .h = 34 }, E.measure(&t, &col, 100));
    try std.testing.expectEqual(Size{ .w = 100, .h = 34 + 2 * section_inset }, E.measure(&t, &sec, 100));
    _ = E.paint(&t, &sec, 10, 10, 100);
    ...
    try std.testing.expectEqual(Size{ .w = 20, .h = 34 + 20 }, E.measure(&t, &sec, 20));
}
...
test "a split stacks when the right pane would be too narrow and rules otherwise" {
    ...
    var t: TestTree = .{};
    try std.testing.expectEqual(Size{ .w = 600, .h = 80 }, E.measure(&t, &s, 600));
    _ = E.paint(&t, &s, 0, 0, 600);
    ...
    var narrow: TestTree = .{};
    try std.testing.expectEqual(Size{ .w = 300, .h = 50 + split_gap + 80 }, E.measure(&narrow, &s, 300));
    _ = E.paint(&narrow, &s, 0, 0, 300);
...
test "a scroll node measures as its viewport and paints through the tree" {
    ...
    var t: TestTree = .{};
    try std.testing.expectEqual(Size{ .w = 300, .h = 240 }, E.measure(&t, &sc, 300));
    try std.testing.expectEqual(Size{ .w = 300, .h = 240 }, E.paint(&t, &sc, 0, 0, 300));
